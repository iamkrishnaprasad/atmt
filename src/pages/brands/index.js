...
import { BRAND_INITIAL_VALUE } from '../../constant';
import BrandModal from './BrandModal';
import { addBrand, updateBrand } from '../../redux';
...
function BrandsPage() {
  ...
  const pageSize = 10;
  const pagesCount = Math.ceil(brands.length / pageSize);
  ...
  const openModal = (type, payload = '') => {
    ...
    switch (type) {
      ...
      case 'add':
      default:
        setFormData(BRAND_INITIAL_VALUE);
        break;
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <>
        ...
        <Col>
          <Widget>
            <div className={styles.tableTitle}>
              <div className="headline-2">Brands</div>
              <div
                onClick={() => {
                  openModal('add');
                }}
            ...
            <div className="widget-table-overflow">
              <Table className={classNames('table-striped table-borderless table-hover', styles.textAlignCenter)} responsive>
                ...
                <tbody>
                  {brands.slice(currentPage * pageSize, (currentPage + 1) * pageSize).map((item, index) => (
                    <tr key={`${item?.id}`}>
      ...
      <BrandModal variant={formType} isOpen={isModalOpen} toggle={toggleModal} data={formData} onSubmit={submitForm} />
    </>
  );
}
